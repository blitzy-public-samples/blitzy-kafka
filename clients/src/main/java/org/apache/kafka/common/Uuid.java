 ...
 *
 * @implNote DECISION: Custom UUID implementation rather than java.util.UUID for two reasons:
 * (1) Base64 URL-safe string representation instead of hex-dash format — more compact for wire
 * transmission and logging; (2) reserved value semantics (ZERO_UUID, ONE_UUID) that java.util.UUID
 * does not support. Alternative: java.util.UUID with custom formatting. Rationale: Kafka uses UUIDs
 * extensively in metadata (topic IDs, directory IDs) where compact representation matters for
 * protocol efficiency. Custom type prevents accidental mixing with java.util.UUID.
 */
// CROSS-CUTTING: Used across all modules for topic identification (KIP-516): Cluster.topicIds(),
// FetchSessionHandler, metadata/PartitionRegistration, storage/Log, coordinator state machines,
// and protocol messages. Also used for directory IDs in JBOD support (KIP-858).
public class Uuid implements Comparable<Uuid> {
     ...
     */
    // DECISION: Alias for ONE_UUID to identify the __cluster_metadata topic in KRaft mode. Using a
    // well-known constant avoids needing a name-to-ID lookup for this system-critical topic.
     ...
     */
    // DECISION: Sentinel representing "no UUID assigned" — used when topic IDs are not yet
    // available (pre-KIP-516 brokers). Checked by Uuid.equals(ZERO_UUID) in protocol handling.
    public static final Uuid ZERO_UUID = new Uuid(0L, 0L);

    /**
     * The set of reserved UUIDs that will never be returned by the randomUuid method.
     */
    // DECISION: Reserved UUIDs (ZERO, ONE) are excluded from randomUuid() to prevent collision
    // with sentinel values. This is a safety invariant — accidental generation of ZERO_UUID would
    // corrupt topic ID mapping.
     ...
     */
    // DECISION: Uses ThreadLocalRandom (via unsafeRandomUuid) rather than SecureRandom.
    // Alternative: SecureRandom. Rationale: UUIDs are identifiers not cryptographic tokens —
    // ThreadLocalRandom provides sufficient uniqueness (variant 2 version 4) with much lower
    // contention than SecureRandom under high-throughput topic creation.
     ...
     */
    // DECISION: URL-safe Base64 (RFC 4648 Section 5) without padding for compact string
    // representation. Alternative: Hex-dash format (like java.util.UUID). Rationale: 22 chars
    // vs 36 chars — significant savings when logging/transmitting thousands of topic IDs.
    @Override
