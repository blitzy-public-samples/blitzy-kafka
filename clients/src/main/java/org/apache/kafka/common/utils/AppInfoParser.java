...
// DECISION: Parses kafka-version.properties from classpath for build metadata (version, commitId).
// Alternative: Compile-time constants via annotation processing. Rationale: Properties file is
// generated by Gradle build, enabling version info without recompilation of this class. Properties
// approach also works in shaded/relocated JARs where class constants might be rewritten.
//
// CROSS-CUTTING: Called by KafkaProducer, KafkaConsumer, KafkaAdminClient, KafkaStreams, and
// Connect Worker during initialization to register version metrics. Depends on common/metrics/
// for metric registration and JMX for MBean exposure.
public class AppInfoParser {
    ...
    // DECISION: Registers a JMX MBean per client instance (keyed by client ID) for version/commit
    // visibility. Alternative: Single global MBean. Rationale: Per-client-ID registration enables
    // operators to see which client version each connection uses — critical for rolling upgrades
    ...
    // DECISION: Registers app-info metrics (version, commit-id) as non-measurable string metrics.
    // These are informational metrics exposed via MetricsReporter for monitoring dashboards.
