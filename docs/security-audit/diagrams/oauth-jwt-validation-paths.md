...
- `clients/src/main/java/org/apache/kafka/common/security/oauthbearer/BrokerJwtValidator.java:L102-L134` — package-visible constructor and `configure(Map, String, List<AppConfigurationEntry>)` method body; the `JwtConsumerBuilder` chain at L130-L135 contains `.setJwsAlgorithmConstraints(DISALLOW_NONE)` at L131, `.setRequireExpirationTime()` at L132, `.setRequireIssuedAt()` at L133, and `.setVerificationKeyResolver(verificationKeyResolver)` at L134.
...
- `clients/src/main/java/org/apache/kafka/common/security/oauthbearer/internals/unsecured/OAuthBearerUnsecuredValidatorCallbackHandler.java:L161-L204` — the `handleCallback(OAuthBearerValidatorCallback)` body (plus the `allowableClockSkewMs()` helper at L194-L207), which constructs `OAuthBearerUnsecuredJws` without any signature check and only runs clock-skew-bounded `issuedAt` / `expirationTime` / `timeConsistency` / `scope` validation.
