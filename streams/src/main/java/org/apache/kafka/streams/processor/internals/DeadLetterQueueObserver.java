 ...
 * Dead Letter Queue (DLQ) topic: recording the {@code dlq-records-sent} sensor and
 * emitting a single WARN log that identifies the failure without exposing the record
 * key/value payload. Invoked from the four DLQ send sites (deserialization, serialization,
 * production, and processing failure paths). All invocations happen on the failure branch
 * only, so this adds no overhead to the non-failing record path.
 */
final class DeadLetterQueueObserver {
     ...
     * only from within a {@code !deadLetterQueueRecords.isEmpty()} branch.
     *
